...
void JsonValue::for_each_object(std::function<void(const std::string&, const JsonValue&)> callback) const {
    if (!is_object()) {
        return;
    }
...
JsonBuilder& JsonBuilder::put_object(const std::string& key, std::function<void(JsonBuilder&)> builder) {
    if (impl_->is_object) {
...
JsonBuilder& JsonBuilder::put_array(const std::string& key, std::function<void(JsonBuilder&)> builder) {
    if (impl_->is_object) {
