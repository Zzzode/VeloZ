...
public:
     ...
     *
     * @tparam T The type to parse as (bool, int, int32_t, int64_t, uint32_t, uint64_t, float, double, std::string)
     ...
     */
    template<typename T>
    T parse_as_or(T default_val) const {
...
public:
     ...
     */
    bool is_valid() const { return val_ != nullptr; }
    ...
    std::optional<T> parse_as() const {
        ...
        if constexpr (std::is_same_v<DecayT, bool>) {
            ...
            if (!is_int() && !is_uint()) return std::nullopt;
            ...
            if (!is_real() && !is_int() && !is_uint()) return std::nullopt;
     ...
     *
     * @tparam T The element type (bool, int, int32_t, int64_t, uint32_t, uint64_t, float, double, std::string)
     * @return Vector of parsed values or empty vector on failure
     */
    template<typename T>
    std::vector<T> parse_as_vector() const {
        using DecayT = std::decay_t<T>;
        std::vector<T> result;
