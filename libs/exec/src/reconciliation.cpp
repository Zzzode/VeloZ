...
kj::Promise<void> AccountReconciler::handle_orphaned_orders(veloz::common::Venue venue,
  ...
  for (auto& order : orphaned) {
    ...
    if (config_.auto_cancel_orphaned && query_interface != nullptr) {
      ...
      chain = chain.then(
          [this, query_interface, symbol, client_order_id = kj::mv(client_order_id)]() mutable {
            return query_interface->cancel_order_async(symbol, client_order_id)
                .then([this, client_order_id =
                                 kj::mv(client_order_id)](kj::Maybe<ExecutionReport> result) {
