...
BinanceAdapter::BinanceAdapter(const std::string& api_key, const std::string& secret_key,
                               bool testnet)
    : api_key_(api_key), secret_key_(secret_key), connected_(false), testnet_(testnet),
      rate_limit_window_(std::chrono::seconds(1)), rate_limit_per_window_(1200), max_retries_(3),
...
std::optional<ExecutionReport> BinanceAdapter::place_order(const PlaceOrderRequest& req) {
  ...
  params << "symbol=" << format_symbol(req.symbol) << "&side=" << order_side_to_string(req.side)
         << "&type=" << order_type_to_string(req.type)
         << "&timeInForce=" << time_in_force_to_string(req.tif) << "&quantity=" << req.qty;
...
std::optional<ExecutionReport> BinanceAdapter::cancel_order(const CancelOrderRequest& req) {
  ...
  std::ostringstream params;
  params << "symbol=" << format_symbol(req.symbol) << "&origClientOrderId=" << req.client_order_id
         << "&timestamp="
         << std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now().time_since_epoch())
                .count();
...
bool BinanceAdapter::is_connected() const {
  ...
  auto idle_time = std::chrono::duration_cast<std::chrono::seconds>(now - last_activity_time_);
