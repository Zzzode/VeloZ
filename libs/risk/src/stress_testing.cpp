...
FactorShock::FactorShock(const FactorShock& other)
    : factor(other.factor), symbol(kj::heapString(other.symbol)), shock_value(other.shock_value),
...
StressPosition::StressPosition(const StressPosition& other)
    : symbol(kj::heapString(other.symbol)), size(other.size), entry_price(other.entry_price),
      current_price(other.current_price), volatility(other.volatility) {}
...
StressScenario::StressScenario(const StressScenario& other)
    : id(kj::heapString(other.id)), name(kj::heapString(other.name)),
      description(kj::heapString(other.description)), type(other.type),
      historical_event(kj::heapString(other.historical_event)),
      historical_start_ns(other.historical_start_ns), historical_end_ns(other.historical_end_ns),
      created_at_ns(other.created_at_ns), created_by(kj::heapString(other.created_by)) {
...
PositionStressResult::PositionStressResult(const PositionStressResult& other)
    : symbol(kj::heapString(other.symbol)), base_value(other.base_value),
      stressed_value(other.stressed_value), pnl_impact(other.pnl_impact),
...
StressTestResult::StressTestResult(const StressTestResult& other)
    : scenario_id(kj::heapString(other.scenario_id)),
      scenario_name(kj::heapString(other.scenario_name)), success(other.success),
      ...
      total_pnl_impact(other.total_pnl_impact), total_pnl_impact_pct(other.total_pnl_impact_pct),
      stressed_var_95(other.stressed_var_95), stressed_var_99(other.stressed_var_99),
      executed_at_ns(other.executed_at_ns), execution_time_us(other.execution_time_us) {
...
SensitivityResult::SensitivityResult(const SensitivityResult& other)
    : factor(other.factor), symbol(kj::heapString(other.symbol)), delta(other.delta),
      gamma(other.gamma) {
...
StressTestResult
StressTestEngine::run_stress_test(kj::StringPtr scenario_id,
...
StressTestResult
StressTestEngine::run_stress_test(const StressScenario& scenario,
                                  const kj::Vector<StressPosition>& positions) const {
  return apply_shocks(scenario, positions);
}

kj::Vector<StressTestResult>
StressTestEngine::run_all_scenarios(const kj::Vector<StressPosition>& positions) const {
...
SensitivityResult StressTestEngine::run_sensitivity_analysis(
    MarketFactor factor, const kj::Vector<StressPosition>& positions, double shock_min,
    double shock_max, int num_points) const {

  return run_sensitivity_analysis(factor, ""_kj, positions, shock_min, shock_max, num_points);
}

SensitivityResult StressTestEngine::run_sensitivity_analysis(
    MarketFactor factor, kj::StringPtr symbol, const kj::Vector<StressPosition>& positions,
    double shock_min, double shock_max, int num_points) const {
...
StressTestEngine::ScenarioComparison
StressTestEngine::compare_scenarios(const kj::Vector<StressTestResult>& results) const {
...
StressTestResult StressTestEngine::apply_shocks(const StressScenario& scenario,
                                                const kj::Vector<StressPosition>& positions) const {
...
double StressTestEngine::calculate_stressed_price(const StressPosition& position,
...
const FactorShock* StressTestEngine::find_shock(const kj::Vector<FactorShock>& shocks,
