...
bool is_valid_time_frame(const std::string& time_frame) {
    static const std::vector<std::string> valid_frames = {
...
std::vector<veloz::market::MarketEvent> CSVDataSource::get_data(
    ...
    try {
        while (std::getline(file, line)) {
            ...
            if (tokens.size() < 5) {
                skipped_lines++;
                logger.warn(std::format("Skipping malformed line {}: insufficient columns (expected 5, got {})",
            ...
            try {
                ...
                if (side == "buy") {
                    ...
                    skipped_lines++;
                    logger.warn(std::format("Skipping malformed line {}: invalid side '{}'", line_number, side));
                    continue;
                }
                ...
                if (trade_data.price <= 0) {
                    skipped_lines++;
                    logger.warn(std::format("Skipping malformed line {}: invalid price '{}'", line_number, tokens[3]));
                    continue;
                }
                ...
                if (trade_data.qty <= 0) {
                    skipped_lines++;
                    logger.warn(std::format("Skipping malformed line {}: invalid quantity '{}'", line_number, tokens[4]));
                    continue;
                }
                ...
                event.payload = std::format(R"({{"type":"trade","symbol":"{}","timestamp":{},"price":{},"quantity":{},"side":"{}"}})",
...
bool CSVDataSource::download_data(
    ...
    if (end_time <= start_time) {
        logger.error(std::format("download_data: end_time ({}) must be greater than start_time ({})", end_time, start_time));
        return false;
    }
    ...
    if (data_type != "trade") {
        logger.error(std::format("download_data: Unsupported data type '{}'. Only 'trade' is supported.", data_type));
        return false;
    }
    ...
    try {
        ...
        if (!output_dir.empty() && !std::filesystem::exists(output_dir)) {
            if (!std::filesystem::create_directories(output_dir)) {
                logger.error(std::format("download_data: Failed to create output directory: {}", output_dir.string()));
                return false;
            }
    ...
    if (!output_file.is_open()) {
        logger.error(std::format("download_data: Failed to open output file for writing: {}", output_file_path.string()));
        return false;
    }
    ...
    for (std::int64_t i = 0; i < total_trades; ++i) {
        ...
        std::int64_t time_increment = avg_trade_interval_ms +
                                      static_cast<std::int64_t>(rng() % 100);  // Add 0-100ms variation
    ...
    if (!output_file.good()) {
        logger.error(std::format("download_data: Error occurred while writing to file: {}", output_file_path.string()));
        return false;
    }

    logger.info(std::format("Successfully generated {} trade records to: {}",
                           records_written, output_file_path.string()));
...
BinanceDataSource::BinanceDataSource()
    ...
    , rate_limit_per_minute_(1200)
    , rate_limit_per_second_(10) {
#ifndef VELOZ_NO_CURL
...
std::vector<veloz::market::MarketEvent> BinanceDataSource::get_data(
    ...
    if (data_type == "kline" && !is_valid_time_frame(effective_time_frame)) {
        logger.error(std::format("Binance API: invalid time frame '{}'. Valid frames: 1s, 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M", effective_time_frame));
        return events;
    }
        ...
        while (current_start_time < end_time || (end_time == 0 && request_count == 0)) {
            ...
            for (int retry = 0; retry < max_retries_; ++retry) {
                ...
                if (retry < max_retries_ - 1) {
                    logger.warn(std::format("Binance API: Request failed, retrying ({}/{})...", retry + 1, max_retries_));
            ...
            try {
                ...
                if (code.is_int() && msg.is_string()) {
                    ...
                    if (code_val == -1003 || code_val == -1021) {
                        logger.warn(std::format("Binance API: Rate limit exceeded (code {}), waiting and retrying...", code_val));
                ...
                if (klines_in_batch < kline_limit) {
                    logger.info(std::format("Binance API: Fetched {} klines in {} requests (batch ended with {} klines)",
    ...
    if (effective_data_type == "trade") {
        ...
        if (start_time > 0) {
            ...
            logger.warn("Binance API: /api/v3/trades endpoint doesn't support time filtering, fetching latest 1000 trades");
        }
        ...
        try {
            ...
            logger.info(std::format("Binance API: Successfully fetched {} trades for {}", events.size(), formatted_symbol));
    ...
    logger.error(std::format("Binance API: Unsupported data type '{}'. Supported types: kline, trade", effective_data_type));
    return events;
#endif
}
...
void BinanceDataSource::rate_limit_wait() {
    ...
    static std::chrono::steady_clock::time_point second_window_start = std::chrono::steady_clock::now();
...
bool BinanceDataSource::download_data(
    ...
    if (data_type != "kline") {
        logger.error(std::format("download_data: Unsupported data type '{}'. Only 'kline' is currently supported.", data_type));
        return false;
    }
    ...
    if (!is_valid_time_frame(time_frame)) {
        logger.error(std::format("download_data: Invalid time frame '{}'. Valid values: 1s, 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M", time_frame));
        return false;
    }
    ...
    auto time_frame_to_ms = [](const std::string& tf) -> std::int64_t {
        if (tf == "1s") return 1000LL;
        if (tf == "1m") return 60LL * 1000LL;
        if (tf == "3m") return 3LL * 60LL * 1000LL;
        if (tf == "5m") return 5LL * 60LL * 1000LL;
        if (tf == "15m") return 15LL * 60LL * 1000LL;
        if (tf == "30m") return 30LL * 60LL * 1000LL;
        if (tf == "1h") return 60LL * 60LL * 1000LL;
        if (tf == "2h") return 2LL * 60LL * 60LL * 1000LL;
        if (tf == "4h") return 4LL * 60LL * 60LL * 1000LL;
        if (tf == "6h") return 6LL * 60LL * 60LL * 1000LL;
        if (tf == "8h") return 8LL * 60LL * 60LL * 1000LL;
        if (tf == "12h") return 12LL * 60LL * 60LL * 1000LL;
        if (tf == "1d") return 24LL * 60LL * 60LL * 1000LL;
        if (tf == "3d") return 3LL * 24LL * 60LL * 60LL * 1000LL;
        if (tf == "1w") return 7LL * 24LL * 60LL * 60LL * 1000LL;
        if (tf == "1M") return 30LL * 24LL * 60LL * 60LL * 1000LL;  // Approximate
    ...
    while (current_start_time < end_time) {
        ...
        if (response.empty()) {
            logger.error(std::format("download_data: Empty response from Binance API for request: {}", url.str()));
            output_file.close();
            return false;
        }
        ...
        try {
            ...
            // Process klines
            // Each kline is an array: [open_time, open, high, low, close, volume, close_time, quote_volume, trades, taker_buy_base, taker_buy_quote, ignore]
            for (size_t i = 0; i < root.size(); ++i) {
                const auto kline = root[i];
                ...
                [[maybe_unused]] double taker_buy_quote = std::stod(kline[10].get_string());  // Quote asset buy volume
                ...
                if (trade1_qty <= 0) trade1_qty = volume / 4.0;
                if (trade2_qty <= 0) trade2_qty = volume / 4.0;

                // Write trade 1
                output_file << trade1_time << ","
                           << formatted_symbol << ","
                           << trade1_side << ","
    ...
    logger.info(std::format("Successfully downloaded {} klines to: {}",
                           total_klines, output_file_path.string()));
